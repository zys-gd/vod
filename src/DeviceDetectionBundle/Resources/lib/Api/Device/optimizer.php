...
require_once dirname(__FILE__).'/TreeOptimized.php';
...
function optimizeDataFile($jsonDataFilePath, $tempDir=null, $force=false) {
    ...
    $config->setUseDataOptimizer(true);
    if ($tempDir) {
...
$messages = array(
    ...
    array(
        ...
        "\tphp optimizer.php -c /custom/temp-directory",
        "\nto optimize data file (default configs):",
        "\tphp optimizer.php -d /path/to/datafile.json",
        "\nto optimize data file (manually set temp directory):",
        "\tphp optimizer.php -d /path/to/datafile.json -t /custom/temp-directory",
        "\nuse -f to overwrite cache if already exists:",
        "\tphp optimizer.php -f /path/to/datafile.json",
        "\tphp optimizer.php -f /path/to/datafile.json -t /custom/temp-directory",
        "\nto analyse the speed and memory footprint difference:",
        "\tphp optimizer.php -a /path/to/datafile.json\n",
        "\tphp optimizer.php -a /path/to/datafile.json -t /custom/temp-directory\n",
    ),
    "< < < \n",
);
