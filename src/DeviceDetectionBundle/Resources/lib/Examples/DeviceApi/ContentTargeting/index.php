<?php
/**
 * Content Targeting Example using the DeviceAtlas DeviceApiWeb
 *
...
if (!$errors) {
    ...
    $vendor       = strtolower($properties->vendor);
    $model        = $properties->marketingName;
    $yearReleased = $properties->yearReleased;
...
function putAdvertise(&$toAdvertise, $keyword1, $keyword2=null) {
	...
	foreach ($ads[$keyword1] as $keyword2 => $value) {
	        $toAdvertise[] = $value;
	}
    } else {
...
if (!$errors) {
    ...
    if ($properties->containsKey('osName')) {
        ...
        $osName = $properties->get('osName')->value();
