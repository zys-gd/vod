...
function displayPropertiesUsagesAsHtml($properties) {
    ...
    try {
        if ($properties->containsKey('displayColorDepth')) {
            $displayColorDepth = $properties->get('displayColorDepth')->asBoolean();
        }
    } catch (Exception $x) {
        print "<tr style=\"border: solid 1px red;\"><td id=\"error\">the value of property displayColorDepth is not boolean using properties->get('displayColorDepth')->asBoolean() throws IncorrectPropertyTypeException <a href=\"https://deviceatlas.com/device-data/properties\">find the property data types at https://deviceatlas.com/device-data/properties</a></td></tr>";
    }
...
/* check for errors */
if ($errorMsg) {
    print '<h2>Errors:</h2><p id="error">'.$errorMsg.'</p>';
